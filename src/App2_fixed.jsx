  ...
  return (
    <div style={{ minHeight: "100vh", display: "flex", fontFamily: "system-ui, sans-serif" }}>
      ...
      <div style={{ width: 460, display: "flex", flexDirection: "column", justifyContent: "center", padding: "60px 52px", background: "#fff" }}>
        ...
        <p style={{ fontSize: 13, color: "#6c757d", marginBottom: 24 }}>สำหรับทีมงานภายในองค์กร</p>
        <Alert type="error" msg={error} />
...
function ReceiptForm({ doc, onBack }) {
  ...
  useEffect(() => {
    ...
    if (doc?._convertFromINV) {
      ...
      const recType = inv.invoice_number_type === "invoice_product" ? "receipt_product" : "receipt_service";
...
function DocNumberPage() {
  ...
  const load = async () => {
    ...
    const [logRes, seqRes] = await Promise.all([
      fetch(`${SUPABASE_URL}/rest/v1/doc_number_log?order=created_at.desc&limit=100&select=*`, { headers }),
  ...
  const handleGenerate = async (docType) => {
    ...
    await generateDocNumber(docType);
    await load();
  ...
  const handleReserve = async (docType) => {
    ...
    if (num) alert(`จองเลขที่ ${num} เรียบร้อย`);
  };

  const handleVoid = async (id, docNumber) => {
    if (!confirm(`ยืนยันการยกเลิกเลขที่ ${docNumber}?`)) return;
    const token = sb.auth.getSession()?.access_token || SUPABASE_ANON_KEY;
  ...
  // Recycle: restore a voided doc number back to "reserved" so it can be reused
  ...
  const statusStyle = {
    used:     { label: "ใช้แล้ว",   bg: "#dbeafe", color: "#1e40af" },
    reserved: { label: "จองไว้",    bg: "#fef3c7", color: "#92400e" },
    voided:   { label: "ยกเลิก",   bg: "#fee2e2", color: "#991b1b" },
  };
  ...
  return (
    <div>
      <h1 style={S.pageTitle}>🔢 ระบบเลขที่เอกสาร</h1>
      <p style={S.pageSub}>จัดการ Running Number สำหรับเอกสารทุกประเภท</p>

      {/* Next number preview cards */}
      <div style={{ display: "grid", gridTemplateColumns: "repeat(4, 1fr)", gap: 14, marginBottom: 24 }}>
        {Object.entries(DOC_CONFIGS).map(([docType, config]) => (
          <div key={docType} style={{ ...S.card, textAlign: "center" }}>
            <div style={{ fontSize: 11, color: "#adb5bd", marginBottom: 4, fontWeight: 600, textTransform: "uppercase", letterSpacing: "0.05em" }}>{config.label}</div>
            <div style={{ fontSize: 22, fontWeight: 800, color: "#0F6E56", letterSpacing: "0.02em", marginBottom: 12 }}>
              {getNextPreview(docType)}
            </div>
            <div style={{ fontSize: 11, color: "#adb5bd", marginBottom: 12 }}>เลขถัดไปที่จะออก</div>
            <div style={{ display: "flex", gap: 6, justifyContent: "center" }}>
              <button onClick={() => handleReserve(docType)} disabled={reserving[docType]}
                style={{ padding: "5px 10px", borderRadius: 6, border: "1px solid #f59e0b", background: "#fffbeb", color: "#92400e", fontSize: 11, fontWeight: 500, cursor: "pointer" }}>
                {reserving[docType] ? "..." : "📌 จองเลข"}
              </button>
            </div>
          </div>
        ))}
      ...
      {/* Manual reserve form */}
      <div style={{ ...S.card, marginBottom: 20 }}>
        <div style={{ fontSize: 14, fontWeight: 600, color: "#343a40", marginBottom: 16 }}>📌 จองเลขที่เอกสาร (สำหรับเอกสารนอกระบบ)</div>
      ...
      <div style={S.card}>
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 16, flexWrap: "wrap", gap: 10 }}>
          <div style={{ fontSize: 14, fontWeight: 600, color: "#343a40" }}>ประวัติเลขที่เอกสาร</div>
          <div style={{ display: "flex", gap: 8 }}>
            <select value={filterType} onChange={e => setFilterType(e.target.value)}
              style={{ ...S.input, width: 180, fontSize: 12, padding: "6px 10px" }}>
              <option value="">ทุกประเภท</option>
              {Object.entries(DOC_CONFIGS).map(([k, v]) => <option key={k} value={k}>{v.label}</option>)}
            </select>
            <select value={filterStatus} onChange={e => setFilterStatus(e.target.value)}
              style={{ ...S.input, width: 130, fontSize: 12, padding: "6px 10px" }}>
              <option value="">ทุกสถานะ</option>
              <option value="used">ใช้แล้ว</option>
              <option value="reserved">จองไว้</option>
              <option value="voided">ยกเลิก</option>
            </select>
            <button onClick={load} style={{ ...S.btn("ghost"), padding: "6px 12px", fontSize: 12 }}>🔄</button>
          </div>
        </div>

        {loading ? <div style={{ textAlign: "center", padding: 40, color: "#adb5bd" }}>กำลังโหลด...</div>
          : filteredLogs.length === 0 ? <EmptyState icon="🔢" text="ยังไม่มีประวัติเลขที่เอกสาร" />
          : (
            <table style={S.table}>
              <thead>
                <tr>
                  ...
                  <th style={S.th}>ประเภท</th>
                  <th style={S.th}>ปี/เดือน</th>
                  <th style={S.th}>Running#</th>
                  <th style={S.th}>สถานะ</th>
                ...
                {filteredLogs.map(log => {
                  ...
                  return (
                    <tr key={log.id} onMouseEnter={e => e.currentTarget.style.background = "#f8f9fa"} onMouseLeave={e => e.currentTarget.style.background = ""}>
                      <td style={S.td}><span style={{ fontWeight: 700, fontSize: 15, color: "#0F6E56", letterSpacing: "0.02em" }}>{log.doc_number}</span></td>
                      <td style={S.td}><span style={{ fontSize: 12, color: "#495057" }}>{config?.label || log.doc_type}</span></td>
                      <td style={S.td}><span style={{ fontSize: 12, color: "#6c757d" }}>20{log.year_2digit}/{log.month_2digit}</span></td>
                      <td style={{ ...S.td, textAlign: "center" }}><span style={{ fontWeight: 500 }}>#{log.running_number}</span></td>
                      <td style={S.td}><span style={{ ...S.badge("blue"), background: st.bg, color: st.color }}>{st.label}</span></td>
                      ...
                      <td style={S.td}>
                        ...
                        {log.status === "used" && log.ref_id && (
                          <span style={{ fontSize: 11, color: "#adb5bd" }}>มีเอกสารแล้ว</span>
                        )}
                        {log.status === "voided" && (
      ...
      {/* Recycle Modal */}
      {recycleModal && (
        <Modal title={`♻️ นำเลขที่ ${recycleModal.doc_number} มาใช้ใหม่`} onClose={() => setRecycleModal(null)}>
          <p style={{ fontSize: 13, color: "#495057", marginBottom: 6 }}>
            เลขนี้เคยถูก Void ไปแล้ว ต้องการนำกลับมาใช้อย่างไร?
          </p>
          <p style={{ fontSize: 12, color: "#6c757d", marginBottom: 20 }}>
            ประเภท: {DOC_CONFIGS[recycleModal.doc_type]?.label || recycleModal.doc_type}
          </p>
          <div style={{ display: "flex", flexDirection: "column", gap: 10, marginBottom: 20 }}>
            <div style={{ border: "1px solid #dee2e6", borderRadius: 10, padding: "14px 16px", cursor: "pointer", background: "#f0fff4" }}
              onClick={() => !recycling && handleRecycle("reserve")}>
              <div style={{ fontSize: 13, fontWeight: 600, color: "#0F6E56", marginBottom: 3 }}>📌 จองเลขนี้กลับมา</div>
              <div style={{ fontSize: 12, color: "#6c757d" }}>เปลี่ยนสถานะเป็น "จองไว้" เพื่อนำไปใช้กับเอกสารในภายหลัง</div>
            </div>
            <div style={{ border: "1px solid #dee2e6", borderRadius: 10, padding: "14px 16px", cursor: "pointer", background: "#eff6ff" }}
              onClick={() => !recycling && handleRecycle("use")}>
              <div style={{ fontSize: 13, fontWeight: 600, color: "#1e40af", marginBottom: 3 }}>✅ ทำเครื่องหมายว่าใช้แล้ว</div>
              <div style={{ fontSize: 12, color: "#6c757d" }}>เปลี่ยนสถานะเป็น "ใช้แล้ว" สำหรับเอกสารที่ออกนอกระบบด้วยเลขนี้แล้ว</div>
            </div>
          </div>
          <div style={{ display: "flex", justifyContent: "flex-end" }}>
            <button style={S.btn("ghost")} onClick={() => setRecycleModal(null)} disabled={recycling}>ยกเลิก</button>
          </div>
